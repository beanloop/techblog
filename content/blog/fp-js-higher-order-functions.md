...
Great, so now we have a new funtion called `add42` (someone quick make a npm package out of it!) that will add the number 42 to any given number passed in. This is the anonymous function we returned doing the work, stored in a variable. It remembers the number 42 passed in to the `x` variable, and when invoked, adds it to the `y` variable. We can now use it like any other function
...
This technique is called [currying](https://en.wikipedia.org/wiki/Currying), and deservers its own blog post in the future.
...
Next, we pass it in to the `map` function that will run for every item in the procuts array. This can also be done inline:
